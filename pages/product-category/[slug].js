...
export async function getStaticProps({ params }) {
  ...
  const categoryObj = categories.value.data.filter(
    (category) => category.name === params.slug
  )[0];
  const autoObj = autos.value.data.filter(
    (auto) => auto.name === params.slug
  )[0];

  let show_products;

  if (typeof categoryObj !== "undefined") {
    show_products = products.value.data.filter(
    ...
    show_products = products.value.data.filter(
      (product) => product.category === autoObj.id
    );
  }
...
const ProductCategory = ({ products, categories, show_products }) => {
  ...
  return (
    <PageLayout>
      <Filter>
        <Products
          products_general={show_products}
