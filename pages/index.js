...
import {
  PageLayout, CategoryCar, Offer, Title, Slider, Products, Clients, ListProductsSection, Brands
} from '../components'
...
export async function getStaticProps() {
  ...
  const slider_images_sort = sliderImages.value.data.sort((a, b) => a.id - b.id)
...
export default function Home({ products, categories, autos, sliderImages, discount_products }) {
  ...
  return (
    <PageLayout title='B&V Group Import | Todo para tu auto al mejor precio ..! | Repuestos, accesorios y más.'>
      ...
      <section className={styles.section_categories}>
        <Title name="Productos con Ofertas Especiales" link="Ir a la Sección Ofertas Especiales" />
        <div className='container'>
          <div className={styles.products_categories}>
            <Products products_general={ofertas_especiales} variant_border={"border_solid"} />
          </div>
        </div>
      </section>
